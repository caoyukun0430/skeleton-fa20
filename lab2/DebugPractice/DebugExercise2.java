...
// Yukun: The provided code has a bug that returns the min, rewrite instead
//        int w = (b - a) >> 31;
...
//        Yukun: The function has no bug, but we can simply do return a + b;
        int x = a, y = b;
        /* If you're stepping into this function, click the
           step out button because you're not going to learn anything. */
        int xor, and, temp;
    ...
    public static int arraySum(int[] x) {
        ...
        int sum = 0;
        while (i < x.length) {
            // Yukun: The bug is sum is added twice for each i
//            sum = sum + add(sum, x[i]);
            sum = add(sum, x[i]);
            i = i + 1;
        }
